...
test("gallery wraps responsively without a horizontal image row", async () => {
  const css = await read("app/globals.css");
  assert.match(css, /\.gallery-grid\s*\{[^}]*display:\s*grid/s);
  assert.match(css, /\.gallery-grid\s*>\s*\*\s*\{[^}]*min-width:\s*0/s);
  assert.match(
    css,
    /@media\s*\(max-width:\s*767px\)[\s\S]*?\.gallery-grid\s*\{[^}]*grid-template-columns:\s*1fr/s,
  );
  assert.match(
    css,
    /@media\s*\(max-width:\s*767px\)[\s\S]*?\.gallery-grid\s*>\s*\*[^}]*\{[^}]*width:\s*100%/s,
  );
});
